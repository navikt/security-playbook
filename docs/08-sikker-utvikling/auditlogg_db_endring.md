...
For eksempel skal vedtak inneholde informasjonen om hvem som har fattet vedtaket og når. 

## Hvordan skru på/bestille auditlogging ?

NB! Auditloggene skal inkludere parametrene i SQL-kommandoen.
...
| **Verifikasjon** | Logg inn som personlig bruker og utfør SQL som er omfattet av bestilt auditlogging (f.eks. `SELECT`).  <br/> En på teamet med lesetilgang til audit-logg-view må logge inn og verifisere at SQL er logget. <br/> Eksempler på spørringer finnes i Confluence-siden [Bestilling av auditlogg i Oracle databaser](https://confluence.adeo.no/spaces/IDD/pages/752096186/Bestilling+av+auditlogg+i+Oracle+databaser). |
